 ...
 * @since  1.6
 */
class JdtoursshowcaseTabletour extends JTable
{
	...
	public function bind($array, $ignore = '')
	{
		...
		{
			if (is_array($array['tour_type']))
			{
				$array['tour_type'] = implode(',',$array['tour_type']);
			}
			elseif (strpos($array['tour_type'], ',') != false)
			{
				$array['tour_type'] = explode(',',$array['tour_type']);
			}
			elseif (strlen($array['tour_type']) == 0)
			{
				$array['tour_type'] = '';
			}
		}
		...
		if ((!empty($array['tour_schedule'])&&(is_array($array['tour_schedule'])))){
			$this->_jsonEncode[] = "tour_schedule";
	  }
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_jdtoursshowcase.tour.' . $array['id']))
		{
			$actions         = JAccess::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_jdtoursshowcase/access.xml',
				"/access/section[@name='tour']/"
			);
			$default_actions = JAccess::getAssetRules('com_jdtoursshowcase.tour.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	protected function _getAssetName()
	{
		...
		return 'com_jdtoursshowcase.tour.' . (int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		$assetParent->loadByName('com_jdtoursshowcase');
